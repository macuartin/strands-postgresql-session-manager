...
def session_manager(engine):
    ...
    return PostgresSessionManager(session_id="integration_test_session", engine=engine)
    ...
    def test_complete_agent_session_lifecycle(self, session_manager, engine):
        ...
        with Session(engine) as db_session:
            agent_db = db_session.get(
                AgentDB, {"session_id": "integration_test_session", "agent_id": "main_agent"}
            )
...
class TestJSONBStorage:
    ...
    def test_complex_state_storage(self, session_manager, engine):
        ...
        with Session(engine) as db_session:
            agent_db = db_session.get(
                AgentDB, {"session_id": "integration_test_session", "agent_id": "agent_main"}
            )
...
class TestPerformance:
    ...
    def test_large_message_volume(self, session_manager, engine):
        ...
        # Test pagination
        page_1 = session_manager.list_messages(
            "integration_test_session", "agent_main", limit=100, offset=0
        )
        page_2 = session_manager.list_messages(
            "integration_test_session", "agent_main", limit=100, offset=100
        )
